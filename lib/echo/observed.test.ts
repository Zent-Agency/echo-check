...
test('compromised trusted source: mark_as_untrusted tightens UNPROVEN -> REJECT', () => {
  // A maintainer account was compromised; its signoff is trusted by policy but
  // attacker-controlled. reviewer-c reads it as "independent" evidence.
  const base = {
    origins: { 'issue-42.md': 'untrusted', 'security-signoff.md': 'trusted' } as const,
    events: [
      ...journal.events,
      { op: 'read' as const, agent: 'reviewer-c', path: 'security-signoff.md' },
      { op: 'emit' as const, agent: 'reviewer-c', id: 'msg-approve-c' },
    ],
  };
  const confirmations = ['msg-approve-a', 'msg-approve-b', 'msg-approve-c'];

  // Without the mark, the compromised signoff launders as independent evidence.
  const vulnerable = evaluate(graphFromJournal(base), 'release-summary.md', confirmations);
  assert.equal(vulnerable.independentSources, 1);
  assert.equal(vulnerable.verdict, 'UNPROVEN');
  // A human, trusting that "1 independent source", could approve -> PASS -> deploy.
  const approved = evaluate(graphFromJournal(base), 'release-summary.md', confirmations, {
    approvedBy: 'operator',
  });
  assert.equal(approved.verdict, 'PASS');

  // A monitor flags the signoff. Downgrade only — the tool cannot grant trust.
  const guarded: Journal = {
    ...base,
    events: [...base.events, { op: 'mark', agent: 'monitor', path: 'security-signoff.md' }],
  };
  const blocked = evaluate(graphFromJournal(guarded), 'release-summary.md', confirmations, {
    approvedBy: 'operator',
  });
  assert.equal(blocked.independentSources, 0);
  // REJECT overrides even the human approval: zero evidence cannot be approved.
  assert.equal(blocked.verdict, 'REJECT');
  assert.equal(canExecute(blocked).allowed, false);
});

test('mark is monotone: no path lets an agent upgrade untrusted -> trusted', () => {
  // The only trust-changing event is a downgrade. Marking issue-42 (already
  // untrusted) is a no-op; there is no event that could make it trusted.
  const g = graphFromJournal({
    origins: { 'issue-42.md': 'untrusted' },
    events: [
      { op: 'read', agent: 'release-agent', path: 'issue-42.md' },
      { op: 'mark', agent: 'attacker', path: 'issue-42.md' },
    ],
  });
  assert.equal(g.nodes.find((n) => n.id === 'issue-42.md')?.origin, 'untrusted');
});
